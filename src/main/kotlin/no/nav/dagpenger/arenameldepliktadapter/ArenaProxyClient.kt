...
class ArenaProxyClient(
    ...
    suspend fun hentInnsendteMeldekort(request: DatadelingRequest): List<ArenaMeldekort> {
        ...
        return runCatching {
            ...
            defaultObjectMapper.readValue<List<ArenaMeldekort>>(response.bodyAsText())
        }.getOrElse {
            sikkerlogger.error(it) { "Kunne ikke hente innsendte meldekort fra url: $urlString for request $request" }
            throw it
        }
    }
}
